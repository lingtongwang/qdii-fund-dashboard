...
export function classify(fundOrName = '', type = '', regions = [], trackingIndex = '', holdings = [], industries = [], benchmark = '') {
    ...
    // 1.4 多元资产 FOF / 全球宏观配置 FOF
    if (type.includes('FOF') || name.includes('FOF') || name.includes('多元配置') || name.includes('聚享')) {
    ...
    // =========================================================================
    // 维度 3：指数型基金精准分类（跟踪指数为第一依据，业绩基准为第二依据）
    // =========================================================================
    if (isIndex && idx && idx !== '--' && idx !== '---') {
        // 3.1 亚太单国别宽基
        if (/日经|东证|nikkei|topix/i.test(idx + ' ' + bm)) return { tab: 'apac', pill: '日本', basis: `指数【${idx}】→日本` };
        if (/印度|india|nifty/i.test(idx + ' ' + bm)) return { tab: 'apac', pill: '印度', basis: `指数【${idx}】→印度` };
        if (/越南|vietnam/i.test(idx + ' ' + bm)) return { tab: 'apac', pill: '越南', basis: `指数【${idx}】→越南` };
        if (/东南亚|泛东南亚/i.test(idx + ' ' + bm)) return { tab: 'apac', pill: '东南亚', basis: `指数【${idx}】→东南亚` };
        if (/中韩/i.test(idx + ' ' + bm)) return { tab: 'apac', pill: '科技/半导体', basis: `指数【${idx}】→中韩半导体` };

        // 3.2 行业与主题指数（优先级高于宽基，防止标普信息科技、标普医疗保健被误归入标普宽基）
        if (/生物科技|医药|医疗|健康|biotech|health|pharma/i.test(idx + ' ' + bm)) {
            return { tab: 'us', pill: '生物医药', basis: `指数【${idx}】→生物医药` };
        }
        if (/消费|高端消费|汽车|新能源|consumer|auto/i.test(idx + ' ' + bm)) {
            return { tab: 'us', pill: '消费/新能源', basis: `指数【${idx}】→消费/新能源` };
        }
        if (/科技|半导体|芯片|信息|互联网|数字经济|tech|semiconductor/i.test(idx + ' ' + bm)) {
            return { tab: 'us', pill: '科技/半导体', basis: `指数【${idx}】→美股科技/半导体` };
        }

        // 3.3 美股纯宽基指数
        if (/纳斯达克100|nasdaq 100|ndx/i.test(idx) || (/纳斯达克|nasdaq/i.test(idx) && !/科技|生物|医疗|消费|信息|芯片/.test(idx))) {
            return { tab: 'us', pill: '纳斯达克', basis: `指数【${idx}】→纳斯达克100宽基` };
        }
        if ((/标准普尔500|标普500|s&p 500|sp500|spx|标准普尔|标普100|s&p 100|usa 50|美国50|标普/i.test(idx)) && !/科技|信息|半导体|芯片|生物|医疗|医药|健康|消费|石油|天然气|能源/.test(idx)) {
            return { tab: 'us', pill: '标普', basis: `指数【${idx}】→标普500宽基` };
        }
    }
